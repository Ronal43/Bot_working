...
def get_user_step(uid):                 #В душе не ебу, нах я это сюда вкрутил, но потом что-то намучу с ним, типа привязки по ИД-у и выдачи не более определенного количества акков/прокси
    if uid in userStep:
        return userStep[uid]
    else:
...
#############
    ...
    @bot.message_handler(func=lambda message: message.text == "На главную")                     #Говорит, сколько вообще акков есть, проверка по файлам, очищает от пустот и хлама, точность 99.9999999%
    def handle_text(message): 
        user_markup = telebot.types.ReplyKeyboardMarkup(True, False)
        #orig_mamba = open('mambaorig.txt', 'r+')
        rab_mamba = open('mamba.txt', 'r+')
...
#############
    ...
    @bot.message_handler(func=lambda message: message.text == "Нужна мамба на Киев")                #Выдача мамб(киев), Выдает не больше, чем есть, минимум 1, при ноле возвращает на главную.
    def handle_text(message):
        #orig_mamba_ua = open('mambaorigua.txt', 'r+')
        rab_mamba_ua = open('mambaua.txt', 'r+')
        mamba_list_ua = (rab_mamba_ua.read())
